...
class Data_jabatan extends MY_Controller {
	private $class_link = 'administrator/struktur_organisasi/data_jabatan';
	private $form_errs = array('idErrCode', 'idErrNm');

	public function __construct() {
		parent::__construct();
		parent::datatables_assets();
		$this->load->model(array('model_auth/m_access', 'model_organisasi/tm_jabatan'));

		$access = $this->m_access->read_user_access('data_jabatan', 'Struktur Organisasi');
	...
	*/
	...
	public function table_data() {
		...
		$data = $this->tm_jabatan->ssp_table();
		echo json_encode(
	...
	public function open_form() {
		...
		$data = $this->tm_jabatan->get_data($id);
	...
	public function send_data() {
		...
		$this->load->model('model_basic/base_query');
		if ($this->input->is_ajax_request()) :
			$this->form_validation->set_rules($this->tm_jabatan->form_rules());
			if ($this->form_validation->run() == FALSE) :
				$str = $this->tm_jabatan->form_warning($this->form_errs);
				$str['confirm'] = 'error';
			else :
				$data['kd_jabatan'] = $this->input->post('txtKd');
				$data['user_code'] = $this->input->post('txtCode');
				$data['nm_jabatan'] = $this->input->post('txtNm');
				$str = $this->base_query->submit_data('tm_jabatan', 'kd_jabatan', 'Data Jabatan', $data);
			endif;
	...
	public function delete_data() {
		...
		if ($this->input->is_ajax_request()) :
			$id = $this->input->get('id');
			$str = $this->base_query->delete_data('tm_jabatan', array('kd_jabatan' => $id), 'Data Jabatan');
    ...
    public function code_check($str) {
		if ($str == 'test' || $str == 'TEST') :
			...
			$this->db->from('tm_jabatan')
				->where(array('user_code' => $str, 'kd_jabatan !=' => $this->input->post('txtKd')));
			$query = $this->db->get();
