...
class Client_type extends MY_Controller {
	private $class_link = 'administrator/client_management/client_type';
	private $form_errs = array('idErrNm');

	public function __construct() {
		parent::__construct();
		parent::datatables_assets();
		$this->load->model(array('model_auth/m_access', 'model_client/tm_client_jenis'));

		$access = $this->m_access->read_user_access('client_type', 'Client Management');
	...
	*/
	...
	public function table_data() {
		...
		$data = $this->tm_client_jenis->ssp_table();
		echo json_encode(
	...
	public function open_form() {
		...
		$data = $this->tm_client_jenis->get_data($id);
	...
	public function send_data() {
		...
		if ($this->input->is_ajax_request()) :
			$this->form_validation->set_rules($this->tm_client_jenis->form_rules());
			if ($this->form_validation->run() == FALSE) :
				$str = $this->tm_client_jenis->form_warning($this->form_errs);
				$str['confirm'] = 'error';
			else :
				$data['kd_client_jenis'] = $this->input->post('txtKd');
				$data['nm_client_jenis'] = $this->input->post('txtNm');
				$data['keterangan'] = $this->input->post('txtKeterangan');
				$str = $this->base_query->submit_data('tm_client_jenis', 'kd_client_jenis', 'Data Type Client', $data);
			endif;
	...
	public function delete_data() {
		...
		if ($this->input->is_ajax_request()) :
			$id = $this->input->get('id');
			$str = $this->base_query->delete_data('tm_client_jenis', array('kd_client_jenis' => $id), 'Data Type Client');
